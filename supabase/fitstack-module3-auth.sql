...
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog AS $$
DECLARE target_id uuid; existing public.gym_members%ROWTYPE;
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Sign in required' USING ERRCODE='42501'; END IF;
  IF NOT EXISTS(SELECT 1 FROM auth.users u WHERE u.id=auth.uid() AND NOT coalesce(u.is_anonymous,false) AND (u.email_confirmed_at IS NOT NULL OR u.phone_confirmed_at IS NOT NULL)) THEN
    RAISE EXCEPTION 'A verified identity is required' USING ERRCODE='42501';
  END IF;
  IF NOT EXISTS(SELECT 1 FROM public.profiles p WHERE p.id=auth.uid() AND length(btrim(p.full_name))>=2 AND lower(btrim(p.full_name))<>'new user') THEN
  ...
  IF FOUND THEN
    ...
    RETURN target_id;
  END IF;
  INSERT INTO public.gym_members(gym_id,profile_id,role) VALUES(target_id,auth.uid(),'member')
