...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'attendance_checked_in_by_tenant_fk'
      and conrelid = 'public.attendance'::regclass
  ) then
    alter table public.attendance
      add constraint attendance_checked_in_by_tenant_fk
      foreign key (gym_id, checked_in_by)
      references public.gym_members(gym_id, id)
      on delete restrict;
  end if;
end
$$;
...
revoke all on function public.manual_member_checkin(uuid, uuid)
from public, anon;

grant execute on function public.manual_member_checkin(uuid, uuid)
